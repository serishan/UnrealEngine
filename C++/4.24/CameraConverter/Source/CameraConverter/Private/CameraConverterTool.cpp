...
{
  UniqueIDArray.Add("BP_CameraPawn");
  UniqueIDArray.Add("TargetPoint");
  UniqueIDArray.Add("slate_anim");
}
...
void UCameraConverterTool::ConvertCamera()
{
  ...
  Player->Initialize(LevelSequence, World->PersistentLevel, Settings, CamSettings);
...
{
  TArray<AActor*> ActorList;
  UGameplayStatics::GetAllActorsWithTag(World, "SequencerActor", ActorList);
  ...
  for (auto it : ActorList)
  {
    ...
    else if (it->GetName().Contains("BP_CameraPawn"))
    {
      ...
      it->GetComponents(UCineCameraComponent::StaticClass(), FoundComponents, true);
      CineCameraComponent = Cast<UCineCameraComponent>(FoundComponents[0]);

      TArray<UActorComponent*> OtherComp;
      it->GetComponents(USceneComponent::StaticClass(), OtherComp, true);
...
#define MOVIE_SCENE InSequence->GetMovieScene()
  ...
  for (auto MasterTrack : MasterTracks)
  {
    ...
    for (auto TrackSection : Sections)
    {
      ...
      else if (SubSequence->IsValidLowLevel() && SubSequence->GetName().Contains("Audio")) continue;
  ...
  for (auto it : Tracks)
  {
    if (it->GetName().Contains("FloatTrack"))
    {
      ...
      else if (Property->GetPropertyName().ToString().Contains("SensorWidth"))
      {
        ...
        CineCamera->GetCineCameraComponent()->Filmback.SensorWidth = SensorWidth;
      }
      ...
      else if (Property->GetPropertyName().ToString().Contains("SensorHeight"))
      {
        ...
        CineCamera->GetCineCameraComponent()->Filmback.SensorHeight = SensorHeight;
      }
...
void UCameraConverterTool::SpawnCineCameraActor()
{
  ...
  SensorWidthTrack->SetPropertyNameAndPath("Sensor Width(Filmback Settings)", "Filmback.SensorWidth");
  ...
  SensorHeightrack->SetPropertyNameAndPath("Sensor Height(Filmback Settings)", "Filmback.SensorHeight");
