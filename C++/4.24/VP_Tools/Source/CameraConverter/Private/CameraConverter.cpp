...
void FCameraConverterModule::Initialize(void)
{
  ...
  StyleSet->SetContentRoot(IPluginManager::Get().FindPlugin("VP_Tools")->GetBaseDir() / TEXT("Resources"));
...
{
  static FString ContentDir = IPluginManager::Get().FindPlugin("VP_Tools")->GetBaseDir() / TEXT("Resources");
